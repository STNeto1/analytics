...
func (wq *WebsiteQuery) QueryUser() *UserQuery {
...
func (wq *WebsiteQuery) WithUser(opts ...func(*UserQuery)) *WebsiteQuery {
	query := (&UserClient{config: wq.config}).Query()
...
func (wq *WebsiteQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Website, init func(*Website), assign func(*Website, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Website)
	for i := range nodes {
		if nodes[i].user_websites == nil {
			continue
		}
		fk := *nodes[i].user_websites
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_websites" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
